...
| D4 | Donation payments | **Paystack** (Inline checkout + dynamically-created Plans for monthly giving) as the primary *integrated* processor — superseded the original PayPal decision on 2026-08-11, no PayPal code had shipped yet so this was a clean swap, not a migration | PayPal Checkout (original decision); Stripe as primary; placeholder/no integration yet |
...
3. **Paystack is the integrated processor; bank transfer, Zelle are informational** — those two remain "instructions displayed on the Give page" rather than automated flows, per the original brief. Card payments not routed through Paystack (if ever needed) are out of scope for V1.
4. **Paystack currency scope (2026-08-11)**: unlike PayPal, Paystack doesn't auto-convert "any currency" — it charges in whatever the merchant account supports. Confirmed scope is **NGN and USD, donor's choice via a toggle on the Give page** — not the free-currency behavior FR-DON-01 originally described (that requirement is updated below to match). `Donations.usdAmount` is only populated when the donation was already made in USD; NGN donations leave it blank rather than show a fabricated exchange-rate conversion, since no live FX rate source is integrated.
5. **Payload's built-in localization** feature is used for the EN/FR/IT/ES multilingual requirement rather than a custom Languages collection — it's a first-class Payload capability, not a workaround.
