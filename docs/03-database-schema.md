...
### Donation
donorName (nullable if anonymous), donorEmail, amount (major unit, e.g. 5000 not 500000 kobo), currency (select: NGN/USD/CAD/EUR/GBP — Paystack rows are always NGN or USD, Stripe rows may be any of the five), usdAmount (nullable — only set when currency is already USD; no live FX conversion for the other four, see docs/00-decisions-log.md), fund (select: tithe/mission-projects/child-sponsorship/offering — renamed 2026-08-12 from general/special-campaign), processor (select: paystack/stripe — which gateway handled this charge), paystackReference (nullable, unique — set only when processor is paystack), paystackSubscriptionCode (nullable, set only for a recurring Paystack Plan charge), stripeSessionId (nullable, unique — set only when processor is stripe; doubles as the reference for both the initial Checkout Session and later renewal Invoice ids), stripeSubscriptionId (nullable, set only for a recurring Stripe subscription charge), status (completed/refunded/failed), createdAt. **Never stores card/Paystack/Stripe credentials** — only the transaction reference (FR-DON-05).
...
- `Donation.paystackReference` / `Donation.stripeSessionId`: unique constraints, one per processor — idempotency guard against the client-side verify step and that processor's webhook both trying to record the same charge (see docs/02-architecture.md §6). Both are nullable; a Stripe row leaving paystackReference NULL (and vice versa) doesn't collide since Postgres unique constraints don't compare NULLs to each other.
